...
class logger():
    ...
    def GET_SCHEMA(self, function):
        def get_schema(self):
            ...
            result = function(self)
            logging.logging.logging.debug(f'the schema was loaded successfully')
            logging.logging.logging.debug(f'schema data {self.schema}')
            return result
        return get_schema
    ...
    def NUMBER_THREAD(self, function):
        def get_thread_number(self):
            result = function(self)
            self.logging.debug(f"Number of threads created: {result}")
            return result
        return get_thread_number
    
    def THREAD_STARTED(self, function):
        def start_thread(self, job):
            self.logging.debug(f"Thread started")
            self.logging.debug(f'job\'s data {job}')
            result = function(self, job)
            self.logging.debug(f"Thread finished")
            return result
        return start_thread
    ...
    def RESULTS(self, function):
        def start(self):
            ...
            self.logging.debug(main)
            self.logging.debug(additionalrecordsinfo)
            self.logging.debug(additionalfieldsinfo)
            return result
        return start
    ...
    def ANONYMIZATION_RECORD(self, function):
        def import_data(self, connection, field, source_table, row_id, primary_key, value):
            ...
            self.logging.info(f'{source_table} {row_id} {field} anonymized -> {value}')
            return result
        return import_data
    ...
    def TRUNCATE_TABLES(self, function):
        def truncate_tables(self, connection):
            ...
            self.logging.info(f'{self.schema} deleted')
            return result
    ...
    def INSERT_MIGRATED_FIELD(self, function):
        def insert_migrated_fields_rec(self, cr, field, table):
            ...
            self.logging.debug(f'INSERT INTO {table} ({field})')
            return result
    ...
    def INSERT_MIGRATED_DATA(self, function):
        def create_anon(self, con, table, data):
            ...
            self.logging.info(f'{migrated_table} new({field} {id} {value})')
            return result
        return create_anon
    ...
    def UPDATE_MIGRATED_DATA(self, function):
        def update_migrated_data_history(self, cr, id, table):
            ...
            migrated_table = constants.TABLE_MIGRATED_DATA+"_"+table
            self.logging.debug(f'UPDATE {migrated_table} (state:1, id:{id})')
            return result
    ...
    def DEANONYMIZATION_RECORD(self, function):
        def revert_anonymization(self, connection, record, table, field, value):
            ...
            self.logging.info(f'{table} {record[0]} {field} deanonymized -> {value}')
            return result
